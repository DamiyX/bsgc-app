...
> Milestone 12 correction: Firestore SDK persistence is enabled in
> `lib/main.dart` to support warm offline study data. Any earlier row text in
> this document that says Firestore persistence is disabled or cleared is
> superseded by that source behavior. Sign-out clears the account-scoped
> drafts, outbox, voice/image caches, routes, and navigation state, but a
> complete Firestore-cache purge is not currently guaranteed while listeners
> are active. Account-switch isolation therefore remains a required
> emulator/physical-device release check.
...
| --- | --- | --- | --- | --- | --- |
| App startup | The shell renders immediately. Firebase initialization has a 15-second bound and offers Retry. A signed-in private profile may be unavailable after a process restart if it was not previously cached. Firestore SDK persistence is enabled for warm offline study data. | In-process screen state remains usable while the process lives. Bundled Bible content remains available. Previously loaded Firestore data may also be available from the SDK cache, subject to the account-isolation verification below. | Not applicable. | Retry re-runs bounded startup. Authentication/profile sections recover independently. | Startup failure is retryable; the UI must not spin forever. |
...
| Message attachments | Uncached remote media is unavailable. | Locally queued attachments and previously authorized managed images/audio remain available within their retention limits. | Supported attachments are copied into an account-scoped outbox before enqueue. Per-file and total quota failures are shown before claiming success. | Upload resumes through the outbox; stable IDs and canonical Storage paths prevent duplicate logical messages. | Missing files become non-retryable visible items instead of disappearing; arbitrary HTTPS media is never auto-loaded. |
| Voice playback | Uncached remote audio cannot play; the UI says it must be downloaded first. | Explicitly downloaded or first-play-cached audio plays from the account-scoped cache. | Authenticated Storage download progress is visible. The shared managed-media cache is capped at 50 MiB per account with 14-day expiry and least-recently-used eviction. | Download/first play may be retried. Cached playback does not require reconnect. | Unsupported format, no authorization, removed file, offline miss, and transient failure have distinct copy and a retry/download action. |
